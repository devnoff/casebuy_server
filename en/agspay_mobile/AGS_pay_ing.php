...
********************************************************************************/
	...
	****************************************************************************/
	...
	foreach($_POST as $key=>$value){
		if (true === mb_detect_encoding($value,'UTF-8')){
			
		}
		
		$val = iconv("UTF-8", "EUC-KR", $value);
			$_POST[$key] = $val;
	...
	/*공통사용*/
	$agspay->SetValue("AgsPayHome",$osdir);			//올더게이트 결제설치 디렉토리 (상점에 맞게 수정)
	...
	****************************************************************************/
	...
	$resultData = array();
	$resultData['payment_method'] = $agspay->GetResult("AuthTy");
	$resultData['state'] = '';
	$resultData['rStoreId'] = $agspay->GetResult("rStoreId");
	$resultData['rOrdNo'] = intVal($agspay->GetResult("rOrdNo"));
	$resultData['rProdNm'] = iconv("EUC-KR", "UTF-8", $agspay->GetResult("rProdNm"));
	$resultData['rAmt'] = $agspay->GetResult("rAmt");
	$resultData['rSuccYn'] = $agspay->GetResult("rSuccYn");
	$resultData['rResMsg'] = iconv("EUC-KR", "UTF-8", $agspay->GetResult("rResMsg"));
	
	// 에스크로 결제시
	$resultData['ES_SENDNO'] = $agspay->GetResult("ES_SENDNO");
	...
	$resultData['rHP_DATE'] = '';
	$resultData['rHP_TID'] = '';
	$resultData['rHP_HANDPHONE'] = '';
	$resultData['rHP_COMPANY'] = '';
	
	// 신용카드
	if($agspay->GetResult("AuthTy") == "card"){
		$resultData['rBusiCd'] = $agspay->GetResult("rBusiCd");
		$resultData['rDealNo'] = $agspay->GetResult("rDealNo");
		$resultData['rApprNo'] = $agspay->GetResult("rApprNo");
		$resultData['rInstmt'] = $agspay->GetResult("rInstmt");
		$resultData['rApprTm'] = $agspay->GetResult("rApprTm");
		$resultData['rCardCd'] = $agspay->GetResult("rCardCd");
	} 
	...
	// 가상계좌
		...
		$resultData['rAuthTy'] = $agspay->GetResult("rAuthTy");
		$resultData['rApprTm'] = $agspay->GetResult("rApprTm");
		$resultData['rVirNo'] = $agspay->GetResult("rVirNo");
		$resultData['VIRTUAL_CENTERCD'] = intVal($agspay->GetResult("VIRTUAL_CENTERCD"));
	
	} 
	
	// 핸드폰 결제
	else if($agspay->GetResult("AuthTy") == "hp"){
		$resultData['rHP_DATE'] = $agspay->GetResult("rHP_DATE");
		$resultData['rHP_TID'] = $agspay->GetResult("rHP_TID");
		$resultData['rHP_HANDPHONE'] = $agspay->GetResult("rHP_HANDPHONE");
		$resultData['rHP_COMPANY'] = $agspay->GetResult("rHP_COMPANY");
	}
...
/* 		echo ("결제가 실패처리되었습니다. [" . $agspay->GetResult("rSuccYn")."]". $agspay->GetResult("rResMsg").". " ); */
	...
	*/
	...
	if (!savePaymentResult($resultData)){
		$cancelReq = true;
		$reason = 'Log write fail';
	}
	else {
		
		if (!updateOrderState($resultData['rOrdNo'],$resultData['state'])){
			$cancelReq = true;
			$reason = 'Order State update fail';	
		}
		
		if ($resultData['state'] == 'DONE'){
			if (!insertPoint($resultData['rOrdNo'])){
				$reason = 'Point insert fail';
				$cancelReq = true;
			}
		} 

		if (!insertPoint($resultData['rOrdNo'],'SPEND_FOR_PAYMENT')){
			$reason = 'Point insert fail';
			$cancelReq = true;	
		}
	...
	if($cancelReq == true)
	{
		...
		$agspay->SetValue("Type", "Cancel"); // 고정
		$agspay->SetValue("CancelMsg", $reason); // 취소사유
		$agspay->startPay();
		
		// 취소 결과 DB 입력
		$resultData['rSuccYn'] = $agspay->GetResult("rSuccYn");
		$resultData['rResMsg'] = $agspay->GetResult("rResMsg");
		$resultData['state'] = 'FAILED';
...
<input type=hidden name=rResMsg value="<?=iconv('EUC-KR','UTF-8', $agspay->GetResult("rResMsg"));?>">	<!-- 결과메시지 -->
...
<input type=hidden name=rCardNm value="<?=iconv('EUC-KR','UTF-8',$agspay->GetResult("rCardNm"));?>">	<!-- (안심클릭,일반사용)카드사명 -->
...
<input type=hidden name=rAquiNm value="<?=iconv('EUC-KR','UTF-8',$agspay->GetResult("rAquiNm"));?>">	<!-- (안심클릭,일반사용)매입사명 -->
