...
// 성공여부 앱으로 통보
var success = '<?=$rSuccYn;?>'=='y'?true:false;

if (success){
	<?php if ($AuthTy == 'virtual')	{ ?>
		document.location = "scomdcom:success:<?=$rOrdNo;?>:<?=$AuthTy;?>:<?=getCenter_cd($VIRTUAL_CENTERCD);?>:<?=$rVirNo;?>";
	<?php } else { ?>
		document.location = "scomdcom:success:<?=$rOrdNo;?>:<?=$AuthTy;?>";
	<?php } ?>
