...
$AGS_HASHDATA = md5($StoreId . $OrdNo . (int)$amt); 
	...
	if(form.Flag.value == "enable"){
		...
		if(Check_Common(form) == true){
			...
			if(document.AGSPay == null || document.AGSPay.object == null){
				...
				alert(form.ProdNm.value); return;
				
				if(MakePayMessage(form) == true){										
					Disable_Flag(form);
					
					var openwin = window.open("<?=site_url('payment/AGS_progress');?>","popup","width=300,height=160"); //"지불처리중"이라는 팝업창연결 부분
...
<form name=frmAGS_pay method=post action="<?=site_url('payment/AGS_pay_ing');?>">
