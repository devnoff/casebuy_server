...
//공통사용
$AuthTy 		= trim( $postData["AuthTy"] );				//결제형태
$SubTy 			= trim( $postData["SubTy"] );				//서브결제형태
$rStoreId 		= trim( $postData["rStoreId"] );			//업체ID
$rAmt 			= trim( $postData["rAmt"] );				//거래금액
$rOrdNo 		= trim( $postData["rOrdNo"] );				//주문번호
$rProdNm 		= trim( $postData["rProdNm"] );			//상품명
$rOrdNm			= trim( $postData["rOrdNm"] );				//주문자명

//소켓통신결제(신용카드,핸드폰,일반가상계좌)시 사용
$rSuccYn 		= trim( $postData["rSuccYn"] );			//성공여부
$rResMsg 		= trim( $postData["rResMsg"] );			//실패사유
$rApprTm 		= trim( $postData["rApprTm"] );			//승인시각

//신용카드공통
$rBusiCd 		= trim( $postData["rBusiCd"] );			//전문코드
$rApprNo 		= trim( $postData["rApprNo"] );			//승인번호
$rCardCd 		= trim( $postData["rCardCd"] );			//카드사코드
$rDealNo 		= trim( $postData["rDealNo"] );			//거래고유번호

//신용카드(안심,일반)
$rCardNm 		= trim( $postData["rCardNm"] );			//카드사명
$rMembNo 		= trim( $postData["rMembNo"] );			//가맹점번호
$rAquiCd 		= trim( $postData["rAquiCd"] );			//매입사코드
$rAquiNm 		= trim( $postData["rAquiNm"] );			//매입사명


//계좌이체
$ICHE_OUTBANKNAME	= trim( $postData["ICHE_OUTBANKNAME"] );		//이체계좌은행명
$ICHE_OUTACCTNO 	= trim( $postData["ICHE_OUTACCTNO"] );			//이체계좌번호
$ICHE_OUTBANKMASTER = trim( $postData["ICHE_OUTBANKMASTER"] );		//이체계좌소유주
$ICHE_AMOUNT 		= trim( $postData["ICHE_AMOUNT"] );			//이체금액

//핸드폰
$rHP_TID 		= trim( $postData["rHP_TID"] );			//핸드폰결제TID
$rHP_DATE 		= trim( $postData["rHP_DATE"] );			//핸드폰결제날짜
$rHP_HANDPHONE 	= trim( $postData["rHP_HANDPHONE"] );		//핸드폰결제핸드폰번호
$rHP_COMPANY 	= trim( $postData["rHP_COMPANY"] );		//핸드폰결제통신사명(SKT,KTF,LGT)

//ARS
$rARS_PHONE = trim( $postData["rARS_PHONE"] );				//ARS결제전화번호

//가상계좌
$rVirNo 		= trim( $postData["rVirNo"] );				//가상계좌번호 가상계좌추가
$VIRTUAL_CENTERCD = trim( $postData["VIRTUAL_CENTERCD"] );	//가상계좌 입금은행코드

//이지스에스크로
$ES_SENDNO	= trim( $postData["ES_SENDNO"] );				//이지스에스크로(전문번호)
...
$AGS_HASHDATA	= trim( $postData["AGS_HASHDATA"] );	
...
<!--
var openwin = window.open("<?=site_url('payment/AGS_progress');?>","popup","width=300,height=160");
openwin.close();
-->
</script>
