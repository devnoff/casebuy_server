...
from 
(select o.id as id, order_code, order_title, members_id, none_member_pass, date_order, format(totalPrice,2) totalPrice, format(delivery_fee,2) delivery_fee,used_point, format(payable_amount,2) payable_amount, order_state, date_modified, last_admin, c_name, d_name, invoice_no  
from 
orders o 
...
orders left join (select payment_method, rOrdNo from payments group by rOrdNo,payment_method) p on orders.id = p.rOrdNo  ";
    ...
    function ordersByMemberSimple($members_id=null, $uuid=null){
        ...
        $this->db->select("id, order_code, members_id, uuid, date_format(date_order, '%Y.%m.%d') date_order, format(payable_amount,2) payable_amount, order_title", false);
    ...
    function ordersByMemberId($members_id=null){
		...
		$sql = "SELECT o.id id, order_code, members_id, none_member_pass, date_format(date_order, '%Y-%m-%d') date_order, totalPrice, delivery_fee, used_point, format(payable_amount,0) payable_amount, o.order_state, customer_text
				FROM orders o
				LEFT JOIN order_states os ON o.order_state = os.key 
				WHERE  members_id=".$members_id." and DATE_SUB(NOW(), INTERVAL 14 DAY) < DATE(date_modified) order by id desc";
    ...
    function ordersOldByMemberId($members_id=null){
		...
		$sql = "SELECT o.id id, order_code, members_id, none_member_pass, date_format(date_order, '%Y-%m-%d') date_order, totalPrice, delivery_fee, used_point, format(payable_amount,0) payable_amount, o.order_state, customer_text
				FROM orders o
				LEFT JOIN order_states os ON o.order_state = os.key 
				WHERE  members_id=".$members_id . " order by id desc";
    ...
    function orderItemsSimple($orders_id=null){
        ...
        $this->db->select('id, orders_id, products_id, product_name, qty,item_price, format(item_total_amount,2) item_total_amount', false);
        $this->db->from('order_items');
        $this->db->where('order_items.orders_id',$orders_id);
