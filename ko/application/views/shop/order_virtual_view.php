			...
			<div class="virtualAccount">
				...
				<p class="message">회원님의 결제를 위해 위와 같이 가상계좌번호가 생성되었습니다 (예금주 (주)페이게이트)</p>
				<ul>
					<li><span>입금 기한</span>&nbsp; <?=$dateLimit->format('Y-m-d H:i:s');?></li>
					<li><span>입금 금액</span>&nbsp; <strong><?=number_format($order->payable_amount);?></strong>원</li>
				</ul>
			</div>
			...
			<div class="finalPrice">
				<ul>
					<li class="a"><span>상품금액</span><strong><?=number_format($order->totalPrice);?><i>원</i></strong></li>
					<li class="split"><span>+</span></li>
					<li class="b"><span>배송료</span><strong><?=number_format($order->delivery_fee);?><i>원</i></strong></li>
					<? if ($order->used_point > 0) { ?>
					<li class="split"><span>-</span></li>
					<li class="m"><span>사용적립금</span><strong><?=number_format($order->used_point);?><i>원</i></strong></li>
					<? } ?>
					<li class="split"><span>=</span></li>
					<li class="c"><span>결제금액</span><strong><?=number_format($order->payable_amount);?><i>원</i></strong></li>
				</ul>
