...
class Products_model extends CI_Model {
    ...
    function sale_products($limit=10,$offset=0,$category_id=null,$sub_category_id=null){
        ...
        $sql = "select 
        id, categories_id, sub_category_id, brands_id, manufacturers_id, origins_id, title, sub_title, format(purchase_price,0) purchase_price, format(regular_price,0) regular_price, dc_rate, format(fixed_dc_amount,0) fixed_dc_amount, point_rate, fixed_point, point_amount, format(sales_price,0) sales_price, format(profit,0) profit, format(profit_after_delivery,0) profit_after_delivery, format(profit_final,0) profit_final, format(delivery_fee,0) delivery_fee , sales_state , date_release, tags, partners_id, extra_info_title1, extra_info_value1, extra_info_title2, extra_info_value2, extra_info_title3, extra_info_value3, pop, hit, new, dc_sale, recomm, product_code, description, ifnull(web_list_img, '/ko/img/empty.png') web_list_img, web_detail_img, app_list_img, app_detail_img, app_description, web_main, app_main, category_name, sub_category_name, sub_categories_id, category_order 
    ...
    function products($limit=10,$offset=0,$category_id=null,$sub_category_id=null,$keyword=null,$state=null, $sub_category_name=null){
        ...
        (select count(*) from caseshop.products en where en.product_code = products.product_code) sync 
        from products products left join 
...
*/
    ...
    function productsByTagSimple($categories_id,$tag,$offset=0,$limit=null){
        ...
        if (strtoupper($tag)=='ALL' || $tag == '모든 상품'){
            $sql = "select id, title, sales_price,concat('$this->resource_host',app_detail_img) thumb, concat('$this->resource_host',app_list_img) image, sales_state "
    ...
    function productSimple($id){
    	$this->db->select("id, title, sales_price, point_rate, fixed_point , ifnull(web_list_img, '/ko/img/empty.png') web_list_img, sales_state", false);
    	$this->db->where('id',$id);
    	$query = $this->db->get('products');
    ...
    function productAppForList($id){
        $this->db->select("id, title, sales_price, product_code ,concat('$this->resource_host', ifnull(app_detail_img, '/ko/img/empty.png')) thumb, sales_state, if(extra_info_value1='',150,extra_info_value1) weight", false);
        $this->db->where('id',$id);
        $query = $this->db->get('products');
    ...
    function productsTopSales($limit=10,$offset=0){
        ...
        $sql = "select id,categories_id, sub_category_id, title,sub_title, format(purchase_price,0) purchase_price ,format(regular_price,0) regular_price, format(sales_price,0) sales_price ,format(profit,0) profit, format(profit_after_delivery,0) profit_after_delivery,format(profit_final,0) profit_final,extra_info_value1,ifnull(web_list_img, '/ko/img/empty.png') web_list_img,web_detail_img,ifnull(order_cnt,0) order_cnt, web_main, app_main    
    ...
    function productsAppMain($limit=10,$offset=0){
        ...
        $sql = "select id,categories_id, sub_category_id, title,sub_title, format(purchase_price,0) purchase_price ,format(regular_price,0) regular_price, format(sales_price,0) sales_price ,extra_info_value1,concat('$this->resource_host', ifnull(app_main_img, '/ko/img/empty.png')) app_detail_img    
        from products 
...
//			var_dump(is_dir($config['upload_path']));
    ...
    function uploadPhotosToEn($fieldName){
        $fileNames = new stdClass;
        $fileInfo = $this->uploadPhoto($filedName);
        if ($fileInfo){
            $fileNames->file_name = $fileInfo['file_name'];
            $fileNames->file_path = '../en/product_images/'.$fileInfo['file_name'];
            $fileNames->width = $fileInfo['image_width'];
            $fileNames->height = $fileInfo['image_height'];
            return $fileNames;
        }
    ...
    function update($productData){
        ...
        if (gettype($_FILES)=='array' && count($_FILES) > 0){
            ...
            if ($row){
                foreach ($_FILES as $col=>$filename){
                    ...
                    if ($row->$col&&!empty($filename['name'])>0){
                        $path = "..".$row->$col;
        ...
        foreach($_FILES as $files => $filesValue){
            if (!empty($filesValue['name'])){
                ...
                $data[$files] = '/ko/product_images/'.$result['file_name'];

                // 영문스토어 업데이트
                $enData[$files] = $data[$files];
            }
        }
        ...
        if (count($enData)>0){
            $this->db->where('id', $data['id']);
            $this->db->update('caseshop.products', $enData);
        }
