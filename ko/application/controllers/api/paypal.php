...
class Paypal extends CI_Controller 
{
	...
	function shippingOptions($country,$wieght){
		...
		foreach ($rows as $data){
			if ($original_weight > 2000){
				...
				$price += $data->$column;
				$result[$data->option]['price'] = $price;	
			} else {
				$result[$data->option]['price'] = $data->$column;	
			}	
		}
	...
	function Set_express_checkout_demo()
	{
		...
		$SECFields = array(
							...
							'callback' => 'http://noff.me', 							// URL to which the callback request from PayPal is sent.  Must start with https:// for production.
		...
		$Payment = array(
						...
						'paymentrequestid' => '',  				// A unique identifier of the specific payment request, which is required for parallel payments. 
						'sellerid' => '', 						// The unique non-changing identifier for the seller at the marketplace site.  This ID is not displayed.
						'sellerusername' => 'Yongnams Store', 				// The current name of the seller or business at the marketplace site.  This name may be shown to the buyer.
						'sellerpaypalaccountid' => ''			// A unique identifier for the merchant.  For parallel payments, this field is required and must contain the Payer ID or the email address of the merchant.
						);
		...
		foreach($cartItems as $cart){
			...
			$Item = array(
					...
					'number' => $product['product_code'], 							// Item number.  127 char max.
