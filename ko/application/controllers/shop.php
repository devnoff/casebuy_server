...
class Shop extends CI_Controller {
	...
	private function emptySession(){
	    ...
	    delete_cookie('casebuy_none_member');
	...
	private function buyNowItem(){
		$item = get_cookie('casebuy_buy_now');
		if ($item){
	...
	private function setBuyNowItem($items){
		delete_cookie('casebuy_buy_now'); 
		
		set_cookie('casebuy_buy_now',json_encode($items),0);
	}
	...
	function order(){
		...
		$shouldRestore = get_cookie('casebuy_shouldRestoreOrderForm');
		if ($shouldRestore || $shouldRestore == 'true' || $shouldRestore == 1){
			$deliveryInfo['restore'] = true;
			$deliveryInfo['orderer'] = get_cookie('casebuy_orderer');
			$deliveryInfo['recipient'] = get_cookie('casebuy_recipient');
			
			delete_cookie('casebuy_shouldRestoreOrderForm');
			delete_cookie('casebuy_orderer');
			delete_cookie('casebuy_recipient');
		...
		if ($members_id){
			...
			$userData['user_point'] = number_format($this->member_points_model->pointsByMember($members_id));
	...
	function orderResult(){
		...
		if ($result == 'success'){
			...
			$this->orders_model->update($updateData);

			$cardAgent = $this->input->post('card_agent');
		...
		// 가상 계좌 결제 - 입금확인 필요
			...
			$updateData['id'] = $orders_id;
			$updateData['order_state'] = 'WAIT_PAYMENT';
			$this->orders_model->update($updateData);

			$accNo = $this->input->post('acc_no');
			$bankCode = $this->input->post('bank_code');
			$dateLimit = $this->input->post('date_limit');

			$insertData['VIRTUAL_CENTERCD'] = $bankCode;
	...
	function join($step = 0){
		...
		if ($step == 0){
			...
			set_cookie('casebuy_join_ref',$ref, 0);
			
		} else if ($step == 1){
			
			$contentData['ref_to'] = get_cookie('casebuy_join_ref');
	...
	private function myPoint(){
		...
		$contentData['sum_point'] = number_format($sum_point);
